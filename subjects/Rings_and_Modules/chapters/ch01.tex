...
\begin{example}
    Consider the rings $\Z[i]$ and $\Z[\sqrt{-3}]$. The ring $\Z[i]$ is a euclidean domain with respect to the euclidean function $N : \Z[i] \setminus \{0\} \to \Z_{\geq 0}$ defined by $N(a+bi) = a^{2} + b^{2}$ for all $a+bi \in \Z[i] \setminus \{0\}$. Thus, $\Z[i]$ is a principal ideal domain, so every irreducible element of $\Z[i]$ is prime. In particular, $1+i$ is an irreducible element of $\Z[i]$, so $1+i$ is prime. On the other hand, the ring $\Z[\sqrt{-3}]$ is not a principal ideal domain; consider $I = (2,1+\sqrt{-3})$. We show that $I \cap \Z = (2)$. Since $2 \in I$, we have $(2) \subseteq I \cap \Z$. For the converse inclusion, let $x \in I \cap \Z$. Then there exist $r,s \in \Z[\sqrt{-3}]$ such that $x = r(1+\sqrt{-3}) + 2s$. 
\end{example}