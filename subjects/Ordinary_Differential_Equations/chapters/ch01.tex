...
\begin{definition}
    For $A \in M_{n}(\F)$ and $\lambda \in \F$, the \eax{generalized eigenspace} of $A$ corresponding to $\lambda$ is defined as $G_{\lambda}(A) = \ker((A - \lambda I)^{n})$.
\end{definition}
This is, of course, a generalization of the eigenspace $E_{\lambda}(A) = \ker(A - \lambda I)$. Note that $E_{\lambda}(A) \subseteq G_{\lambda}(A)$. We have the following theorem. Further, if all eigenvalues are distinct, then $V = \bigoplus_{i=1}^{m} E_{\lambda_{i}}(A)$. We will also show that $V = \bigoplus_{i=1}^{m} G_{\lambda_{i}}(A)$, for any $A$.